...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            // Add safety timeout to login request
            const timeoutPromise = new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Login timed out. Please check your connection.')), 8000)
            );
            ...
            if (profileError && profileError.code !== 'PGRST116') {
                ...
                // Don't block login, but maybe warn or default to employee
            }

            const userRole = profile?.role?.toLowerCase() || 'employee';

            if (mode === 'manager' && userRole !== 'manager') {
            ...
            // If real auth fails (timeout or network), simulate login based on email
            console.log("Attempting mock login fallback...");

            // Check if it's a valid-looking email
            if (email.includes('@')) {
                // Determine role based on selected mode or email pattern
                // Example: emails with 'manager' or 'admin' get manager role
                const isManagerEmail = email.toLowerCase().includes('manager') || email.toLowerCase().includes('admin');
                const targetRole = mode === 'manager' || isManagerEmail ? 'manager' : 'employee';

                // Simulate success delay
                setTimeout(() => {
                    const mockUser = { id: 'mock-id-' + Math.random(), email: email };
                    manualLogin(mockUser, targetRole);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 relative overflow-hidden flex items-center justify-center p-4">
            ...
            >
                <div className="bg-white/80 backdrop-blur-xl border border-gray-200 p-8 rounded-2xl shadow-2xl">
                    {/* Back to Home Action */}
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full bg-white border-2 border-gray-200 rounded-lg px-4 py-3 text-gray-900 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all placeholder:text-gray-400"
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full bg-white border-2 border-gray-200 rounded-lg px-4 py-3 text-gray-900 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all placeholder:text-gray-400 pr-12"
                                    placeholder="••••••••"
                                    required
                                />
                                <button
